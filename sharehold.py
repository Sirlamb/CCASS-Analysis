...
def validate_arg_date(date):
    pattern_str = r'^\d{4}/\d{2}/\d{2}$'    # "date: YYYY/MM/DD"
    try:
        if re.match(pattern_str, date) and bool(datetime.strptime(date, '%Y/%m/%d')):
            return date
        else:
            raise ValueError('Invalid Shareholding Date')
    except ValueError:
...
#                 example: python CCASS.py -c 941 -s 2023/10/01 -e 2024/01/02 --lang c --sort change
                                        
#                 you can also request for instructions by: python CCASS.py -h 
...
#             for entry_json_end in json_end:
#                 if entry_json_start["id"] == entry_json_end["id"] and entry_json_start["name"] == entry_json_end["name"]:
#                     entry_json_start["is_matched"] = 'Y'
#                     entry_json_end["is_matched"] = 'Y'
#                     shareholding_start = int(entry_json_start["shareholding"].replace(',',''))
#                     shareholding_end = int(entry_json_end["shareholding"].replace(',',''))
#                     shareholding_change = shareholding_end - shareholding_start
#                     shareholding_change_percent = shareholding_change / shareholding_start * 100
